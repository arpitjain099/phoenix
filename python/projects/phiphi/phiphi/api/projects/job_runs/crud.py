...
def check_valid_gather(session: Session, project_id: int, gather_id: int) -> bool:
    ...
    gather = gather_crud.get_gather(session, project_id, gather_id)
    if gather is None:
...
def invalid_foreign_object_guard(
    ...
    if foreign_job_type == schemas.ForeignJobType.gather:
        check_valid_gather(session, project_id, foreign_id)
