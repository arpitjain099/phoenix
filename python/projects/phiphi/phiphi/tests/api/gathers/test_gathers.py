...
def test_get_apify_gathers(client: TestClient, reseed_tables) -> None:
    ...
    response = client.get("/projects/1/gathers/apify")
...
def test_get_apify_gather(client: TestClient, reseed_tables) -> None:
    """Test getting gathers."""
    response = client.get("/projects/1/gathers/apify/1")
    assert response.status_code == 200
    gather_1 = response.json()

    response_2 = client.get("/projects/2/gathers/apify/3")
    ...
    response_3 = client.get("/projects/1/gathers/apify/2")
...
def test_create_get_apify_gather(reseed_tables, client: TestClient) -> None:
    ...
    response = client.post("/projects/1/gathers/apify", json=data)
    assert response.status_code == 200
    ...
    response = client.get(f"/projects/1/gathers/apify/{gather['id']}")
    assert response.status_code == 200
