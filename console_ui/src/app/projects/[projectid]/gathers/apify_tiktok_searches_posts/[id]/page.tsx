...
import { searchPostBaseLink } from "src/utils/constants";
...
export default function ApifyTiktokSearchesPostShow(): JSX.Element {
	...
	return (
		<Show
			...
			headerButtons={() => (
				<GatherViewHeaderButton
					resource="apify_tiktok_searches_posts"
					id={id as string}
		...
		>
			<TextField
				value={translate("gathers.types.apify_tiktok_searches_posts.view.text")}
			/>
				...
				>
					<Accordion.Item value="status" className="mb-4">
						<Accordion.Control>
							<Title order={5}>
								{translate(
									"gathers.types.apify_tiktok_searches_posts.view.accordion.status"
								)}
							</Title>
						</Accordion.Control>
						<Accordion.Panel>
					...
					<Accordion.Item value="general" mb="md">
						<Accordion.Control>
							<Title order={5}>
								{translate(
									"gathers.types.apify_tiktok_searches_posts.view.accordion.general"
								)}
							</Title>
						</Accordion.Control>
						<Accordion.Panel>
							<Container className="mx-0 flex flex-col my-4">
								...
								<Group>
									...
									<TextField
										className="capitalize"
										value={translate("gathers.fields.source.search_posts")}
									/>
								</Group>
								<Group>
									<Title my="xs" order={5}>
									...
									<NumberField value={record?.limit_posts_per_search} />
								</Group>
								<Group>
									<Title my="xs" order={5}>
										{translate(
					...
					<Accordion.Item value="source" mb="md">
						<Accordion.Control>
							<Title order={5}>
								{translate(
									`gathers.types.apify_tiktok_searches_posts.view.accordion.source`
								)}
							</Title>
						</Accordion.Control>
						<Accordion.Panel>
							<Container className="mx-0 flex flex-col my-4">
								<Group>
									<Title my="xs" order={5}>
										{translate(
											`gathers.types.apify_tiktok_searches_posts.view.accordion.source_title`
										)}
										:
									</Title>
